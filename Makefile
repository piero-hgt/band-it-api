...
##	@$(PHP_CONT_TEST) bin/console --no-interaction doctrine:migrations:migrate
	@$(PHP_CONT_TEST) bin/phpunit $(if $(ARGS),$(ARGS),) $(VERBOSECMD)
...
test-static: ## Find errors in your code without actually running it. It catches whole classes of bugs even before you write tests for the code.
	@$(PHP_CONT) vendor/bin/psalm --no-cache --show-info=true $(if $(ARGS),$(ARGS),) $(VERBOSECMD)
...
ci: test-phpunit test-static test-phpcs ## Continuous integration commands
